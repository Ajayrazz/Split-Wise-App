  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.response?.data?.error?.fields) {
        ...
        setErrors({ non_field_errors: ['Registration failed. Please check your inputs.'] });
      }
    } finally {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5 relative z-20">
      ...
      <div className="space-y-1 text-left">
        ...
        {errors.username && <span className="text-xs text-rose-400 ml-1">{errors.username[0]}</span>}
      </div>
